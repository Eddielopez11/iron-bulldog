...
export default React.createClass({
  ...
  leagueAccountSearchHandler(e) {
    ...
    $.getJSON( `https://na.api.pvp.net/api/lol/na/v1.4/summoner/by-name/${this.state.summoner}?api_key=RGAPI-88ae0def-8e60-4e5e-8e0e-998412963d34`, {}).done((data)=>{
      ...
      var newSummonerStateName = this.state.usersLeagueCharacters.currentSummonerState.name
      var newSummonerStateLevel = this.state.usersLeagueCharacters.currentSummonerState.summonerLevel
      this.setState({
        newSummonerStateName: currentSummonerState.name,
        newSummonerStateLevel: currentSummonerState.summonerLevel
      })
      firebase.auth().onAuthStateChanged((user)=>{
        if (user) {
          // User is signed in.
