    ...
    firebase.auth().onAuthStateChanged((user)=>{
      if (user) {
        ...
        firebase.database().ref("/userProfiles/" + user.uid).once("value").then((snapshot)=>{
          ...
          var xboxProfile = this.state.currentUser.xboxAccounts[Object.keys(this.state.currentUser.xboxAccounts)]
          this.setState({
    ...
    $.getJSON( `https://na.api.pvp.net/api/lol/na/v1.4/summoner/by-name/${this.state.summoner}?api_key=RGAPI-88ae0def-8e60-4e5e-8e0e-998412963d34`, {}).done((data)=>{
      ...
      firebase.auth().onAuthStateChanged((user)=>{
        if (user) {
          ...
          $.getJSON(`https://na.api.pvp.net/championmastery/location/NA1/player/23507876/champions?api_key=RGAPI-88ae0def-8e60-4e5e-8e0e-998412963d34`, {}).done((data)=>{
            console.log(data);
          })
        } else {
          // No user is signed in.
  ...
  render() {
    return (
      <div>
        ...
        <h2 className="profile__pageHeader">Your Profile</h2>
        ...
        <div className="profile__accountSection">
          ...
          <div>
            ...
            <h2 className="profile__xboxGamertag">{this.state.currentUserXbox.gamertag}</h2>
            <ol className="profile__topXboxGames">
              <h3 className="profile__recentGamesHeader"></h3>
            {
            ...
            <form className="editProfile__xboxForm" onSubmit={this.xboxGamertagHandler}>
        ...
        <div className="profile__accountSection">
          ...
          <div>
            <form className="editProfile__leagueForm" onSubmit={this.leagueAccountSearchHandler}>
        ...
        <div className="profile__accountSection">
          <h2 className="profile__accountHeader">World of Warcraft
            Character</h2>
          <div>
            <form className="editProfile__wowForm" onSubmit={this.wowCharacterSearchHandler}>
